...
(function (global, factory) {
    ...
    webix.html = {
        ...
        getTextHeightSize: function (text, width, css) {
            var d = webix.html.create("DIV", { "class": "webix_view webix_measure_size " + (css || "") }, "");
            d.style.cssText = "width:" + width + "px; height:1px; visibility:hidden; position:absolute; top:0px; left:0px; word-wrap:break-word; word-break:break-all;";
            document.body.appendChild(d);
    ...
    (function () {
        ...
        function next_timer_call() {
            ...
            if (config.multicall && megapack.length)
                call_remote_async(megaurl, { data: megapack, key: token, multicall: true },
                    megaqueue
                );
        }
    ...
    webix.ajax.prototype = {
        ...
        _send: function (url, params, call, mode) {
            ...
            call.push({
                success: function (t, d) { defer.resolve(d); },
        ...
        _data: function (x) {
            return {
                xml: function () {
                    try { return webix.DataDriver.xml.tagToObject(webix.DataDriver.xml.toObject(x.responseText, this)); } catch (e) {
                        webix.log(x.responseText);
                        webix.log(e.toString());
                        webix.assert_error("Invalid xml data for parsing");
                    }
                },
                rawxml: function () {
        ...
        */
        ...
        _mouseEvent: function (e, hash, name, pair) {
            ...
            if (webix.env.isIE8) {
                ...
                if (!wid) trg.w_view = vid;
                else if (wid !== vid) return;
            }
    ...
    webix.MapCollection = {
        ...
        _process_single_map: function (id, map, extra) {
            ...
            if (map.indexOf("(date)") === 0) {
                start = "webix.i18n.parseFormatDate(";
                end = ")";
    ...
    webix.DataStore.prototype = {
        ...
        sync: function (source, filter, silent) {
            ...
            this._sync_events = [
                ...
                source.attachEvent("onIdChange", webix.bind(function (old, nid) {
    ...
    */
    webix.TreeRenderStack = {
        ...
        _toHTMLLevel: function (id) {
            ...
            if (leaves) {
                html += "<div class='webix_tree_leaves " + this.config.skin.webix_tree_leaves + "'>";
    ...
    webix.SelectionModel = {
        ...
        getSelectedId: function (as_array) {
            switch (this._selected.length) {
                ...
                default:
                    return ([].concat(this._selected)); //isolation
            }
        },
    ...
    webix.EditAbility = {
        ...
        _init_editor: function (id, type, show) {
            ...
            if (show !== false)
                this.showItem(id);
            if (!type.$inline)
                this._sizeToCell(id, node, true, config);
        ...
        _sizeToCell: function (id, node, inline, config) {
            //fake inputs
            if (!node.style) return;

            var pos = this._cellPosition(id);
            if (config && config.editor_margin_height) {
                pos.top += config.editor_margin_height;
                pos.height -= config.editor_margin_height;
            }
            ...
            for (i = 0; i < mask.length; i++) {
                switch (mask[i]) {
                    ...
                    case "%s":
                        ...
                        break;
                    case "%M":
                        splt += "set[1]=webix.i18n.calendar.monthShort_hash[temp[" + i + "]]||0;";
                        break;
                    case "%F":
                        splt += "set[1]=webix.i18n.calendar.monthFull_hash[temp[" + i + "]]||0;";
                        break;
                    case "%c":
    ...
    */
    ...
    webix.ui.datafilter = {
        ...
        "masterCheckbox": {
            ...
            render: function (master, config) {
                var id = "";
                if (master._settings && master._settings.id && config.columnId)
                {
                    id = master._settings.id + "_" + config.columnId;
                }
                ...
                result += " type='checkbox' " + (config.checked ? "checked='1'" : "") + ">";

                return result;
            },
            setCheckValue: function (master, config, checked){
                if(config.id && checked != config.checked)
                {
                    config.checked = checked;
                    document.getElementById(config.id).checked = checked;
    ...
    webix.ProgressBar = {
        ...
        showProgress: function (config) {
            ...
            if (!this._progress) {
                ...
                var progress_class = { "class": "webix_progress_" + config.type };
                if (config.skin) {
                    progress_class.class += webix.getSkinCss(progress_class.class, config.skin);
                }
    ...
    (function () {
        ...
        (function () {
            ...
            function info(text) {
                if (!t.area) {
                    ...
                    t.area.style[t.position] = "105px";
                    document.body.appendChild(t.area);
                }
                t.hide(text.id);
                var message = document.createElement("DIV");
                message.innerHTML = "<div>" + "<span class='webix_icon iconfont icon-" + text.type + "-toast'></span><span class='webix_info_text'>" + text.text + "</span></div>";
        ...
        var mouse = webix.env.mouse = {
            down: "mousedown",
            up: "mouseup",
            move: "mousemove",
            context: t._get_context_m
        };
