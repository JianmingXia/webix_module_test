...
webix.protoUI({
    ...
    setHeaderCheckValue: function (id, checked) {
        for (var index in this._active_headers) {
            if (this._active_headers[index] && this._active_headers[index].columnId == id) {
                var config = this._active_headers[index];
                var type = webix.ui.datafilter[config.content];

                if (type.setCheckValue) return type.setCheckValue(this, config, checked);

                break;
            }
        }
    },
    isHeaderChecked: function (id) {
        for (var index in this._active_headers) {
            if (this._active_headers[index] && this._active_headers[index].columnId == id) {
                var config = this._active_headers[index];
    ...
    markSorting:function(column, order){
        ...
        if (order){
            ...
            if (cell){
                this._sort_sign.className = "webix_icon iconfont webix_ss_sort_"+order;
                this._sort_sign.className += this._addSkinCss("webix_ss_sort_"+order);
