	...
	defaults:{
		template:function(obj, common){
			var text = common.$renderInput(obj, common);
			 if (obj.badge) {
        		text = text.replace("</button>", "<span class='webix_badge" + webix.getSkinCss("webix_badge", obj.skin) + "'>"+obj.badge+"</span></button>");
      		}
...
//		this._settings.label = value;
	...
	_calc_size:function(config){
		...
		if(config.autoheight){
			config.height = webix.html.getTextHeightSize((config.value||config.label), config.width, config.autocss);
		}
	},
